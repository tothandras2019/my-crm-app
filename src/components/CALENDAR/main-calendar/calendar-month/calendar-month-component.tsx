      ...
      <div className='days-container'>
        {placeholderDaysBegining &&
          placeholderDaysBegining.map(
            (value, index, arr) => index > 0 && <Day key={`${value}_${index}_p`} value={value} year={0} monthNo={0} dayNo={index} />,
          )}

        {countDaysArray &&
          countDaysArray.map((value, index, arr) => <Day key={`${value}_${index}_d`} value={value} year={year} monthNo={nth} dayNo={index} />)}
      </div>
    </div>
  );
};
