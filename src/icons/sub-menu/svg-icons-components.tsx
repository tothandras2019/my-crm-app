import { useState } from 'react'

const pathColor: string = '#317d65'

export const LifecycleSvg = () => {
  const [fill, setFill] = useState({ color: pathColor })
  return (
    <svg xmlns='http://www.w3.org/2000/svg' height='24' width='24'>
      <path
        fill={fill.color}
        d='M12.05 20q-3.35 0-5.7-2.325T4 12v-.175l-1.6 1.6-1.4-1.4 4-4 4 4-1.4 1.4-1.6-1.6V12q0 2.5 1.763 4.25Q9.525 18 12.05 18q.65 0 1.275-.15.625-.15 1.225-.45l1.5 1.5q-.95.55-1.95.825-1 .275-2.05.275ZM19 15.975l-4-4 1.4-1.4 1.6 1.6V12q0-2.5-1.762-4.25Q14.475 6 11.95 6q-.65 0-1.275.15-.625.15-1.225.45l-1.5-1.5q.95-.55 1.95-.825Q10.9 4 11.95 4q3.35 0 5.7 2.325T20 12v.175l1.6-1.6 1.4 1.4Z'
      />
    </svg>
  )
}

export const StatusSvg = () => {
  const [fill, setFill] = useState({ color: pathColor })

  return (
    <svg xmlns='http://www.w3.org/2000/svg' height='24' width='24'>
      <path
        fill={fill.color}
        d='m16.7 15.5 3.8-3.25 3 .25-4.4 3.825L20.4 22l-2.55-1.55Zm-2.35-7.3L13.3 5.75 14.45 3l2.3 5.425Zm-7 9.625 3.15-1.9 3.15 1.925-.825-3.6 2.775-2.4-3.65-.325-1.45-3.4L9.05 11.5l-3.65.325 2.775 2.425ZM4.325 22l1.625-7.025L.5 10.25l7.2-.625L10.5 3l2.8 6.625 7.2.625-5.45 4.725L16.675 22 10.5 18.275Zm6.175-8.5Z'
      />
    </svg>
  )
}

export const IdNoSvg = () => {
  const [fill, setFill] = useState({ color: pathColor })

  return (
    <svg xmlns='http://www.w3.org/2000/svg' height='24' width='24'>
      <path
        fill={fill.color}
        d='M3.25 9.65q-.175-.125-.212-.313Q3 9.15 3.15 8.95q1.55-2.125 3.887-3.3 2.338-1.175 4.988-1.175 2.65 0 5 1.137Q19.375 6.75 20.95 8.9q.175.225.112.4-.062.175-.212.3t-.35.112q-.2-.012-.35-.212-1.375-1.95-3.537-2.988-2.163-1.037-4.588-1.037-2.425 0-4.55 1.037Q5.35 7.55 3.95 9.5q-.15.225-.35.25-.2.025-.35-.1Zm11.6 12.325q-2.6-.65-4.25-2.588-1.65-1.937-1.65-4.737 0-1.25.9-2.1.9-.85 2.175-.85t2.175.85q.9.85.9 2.1 0 .825.625 1.388.625.562 1.475.562.85 0 1.45-.562.6-.563.6-1.388 0-2.9-2.125-4.875T12.05 7.8q-2.95 0-5.075 1.975t-2.125 4.85q0 .6.113 1.5.112.9.537 2.1.075.225-.012.4-.088.175-.288.25-.2.075-.388-.013-.187-.087-.262-.287-.375-.975-.537-1.938-.163-.962-.163-1.987 0-3.325 2.413-5.575 2.412-2.25 5.762-2.25 3.375 0 5.8 2.25t2.425 5.575q0 1.25-.887 2.088-.888.837-2.163.837t-2.187-.837Q14.1 15.9 14.1 14.65q0-.825-.612-1.388-.613-.562-1.463-.562-.85 0-1.463.562-.612.563-.612 1.388 0 2.425 1.438 4.05 1.437 1.625 3.712 2.275.225.075.3.25.075.175.025.375-.05.175-.2.3-.15.125-.375.075ZM6.5 4.425q-.2.125-.4.062-.2-.062-.3-.262-.1-.2-.05-.363.05-.162.25-.287 1.4-.75 2.925-1.15 1.525-.4 3.1-.4 1.6 0 3.125.387 1.525.388 2.95 1.113.225.125.263.3.037.175-.038.35t-.25.275q-.175.1-.425-.025-1.325-.675-2.738-1.038-1.412-.362-2.887-.362-1.45 0-2.85.337-1.4.338-2.675 1.063Zm2.95 17.2q-1.475-1.55-2.262-3.163Q6.4 16.85 6.4 14.65q0-2.275 1.65-3.838Q9.7 9.25 12.025 9.25t4 1.562Q17.7 12.375 17.7 14.65q0 .225-.138.362-.137.138-.362.138-.2 0-.35-.138-.15-.137-.15-.362 0-1.875-1.388-3.138-1.387-1.262-3.287-1.262t-3.263 1.262Q7.4 12.775 7.4 14.65q0 2.025.7 3.437.7 1.413 2.05 2.838.15.15.15.35 0 .2-.15.35-.15.15-.35.15-.2 0-.35-.15Zm7.55-1.7q-2.225 0-3.863-1.5-1.637-1.5-1.637-3.775 0-.2.137-.35.138-.15.363-.15.225 0 .363.15.137.15.137.35 0 1.875 1.35 3.075 1.35 1.2 3.15 1.2.15 0 .425-.025t.575-.075q.225-.05.388.062.162.113.212.338.05.2-.075.35-.125.15-.325.2-.45.125-.787.138-.338.012-.413.012Z'
      />
    </svg>
  )
}

export const ContractIconHandShakeSvg = () => {
  const [fill, setFill] = useState({ color: pathColor })

  return (
    <svg xmlns='http://www.w3.org/2000/svg' height='24' width='24'>
      <path
        fill={fill.color}
        d='M11.875 20q.1 0 .2-.05t.15-.1l8.2-8.2q.3-.3.438-.675.137-.375.137-.75 0-.4-.137-.763-.138-.362-.438-.637l-4.25-4.25q-.275-.3-.637-.438Q15.175 4 14.775 4q-.375 0-.75.137-.375.138-.675.438l-.275.275 1.85 1.875q.375.35.55.8.175.45.175.95 0 1.05-.712 1.762-.713.713-1.763.713-.5 0-.962-.175-.463-.175-.813-.525L9.525 8.4 5.15 12.775q-.075.075-.113.163-.037.087-.037.187 0 .2.15.362.15.163.35.163.1 0 .2-.05t.15-.1l3.4-3.4 1.4 1.4-3.375 3.4q-.075.075-.113.162-.037.088-.037.188 0 .2.15.35.15.15.35.15.1 0 .2-.05t.15-.1l3.4-3.375 1.4 1.4-3.375 3.4q-.075.05-.112.15-.038.1-.038.2 0 .2.15.35.15.15.35.15.1 0 .188-.038.087-.037.162-.112l3.4-3.375 1.4 1.4-3.4 3.4q-.075.075-.112.162-.038.088-.038.188 0 .2.163.35.162.15.362.15Zm-.025 2q-.925 0-1.637-.613-.713-.612-.838-1.537-.85-.125-1.425-.7-.575-.575-.7-1.425-.85-.125-1.412-.712-.563-.588-.688-1.413-.95-.125-1.55-.825-.6-.7-.6-1.65 0-.5.188-.963.187-.462.537-.812l5.8-5.775L12.8 8.85q.05.075.15.112.1.038.2.038.225 0 .375-.137.15-.138.15-.363 0-.1-.037-.2-.038-.1-.113-.15L9.95 4.575q-.275-.3-.638-.438Q8.95 4 8.55 4q-.375 0-.75.137-.375.138-.675.438L3.6 8.125q-.225.225-.375.525-.15.3-.2.6-.05.3 0 .612.05.313.2.588l-1.45 1.45q-.425-.575-.625-1.263Q.95 9.95 1 9.25q.05-.7.35-1.363.3-.662.825-1.187L5.7 3.175Q6.3 2.6 7.038 2.3 7.775 2 8.55 2t1.512.3q.738.3 1.313.875l.275.275.275-.275q.6-.575 1.337-.875Q14 2 14.775 2q.775 0 1.513.3.737.3 1.312.875L21.825 7.4q.575.575.875 1.325.3.75.3 1.525 0 .775-.3 1.512-.3.738-.875 1.313l-8.2 8.175q-.35.35-.813.55-.462.2-.962.2ZM9.375 8Z'
      />
    </svg>
  )
}

export const SearchSvg = () => (
  <svg xmlns='http://www.w3.org/2000/svg' height='24px' viewBox='0 0 24 24' width='24px' fill='#FFFFFF'>
    <path d='M0 0h24v24H0V0z' fill='none' />
    <path
      className='search-icon'
      d='M15.5 14h-.79l-.28-.27C15.41 12.59 16 11.11 16 9.5 16 5.91 13.09 3 9.5 3S3 5.91 3 9.5 5.91 16 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z'
    />
  </svg>
)

export const AddSvg = () => (
  <svg xmlns='http://www.w3.org/2000/svg' height='36px' viewBox='0 0 24 24' width='36px' fill='#FFFFFF'>
    <path d='M0 0h24v24H0V0z' fill='none' />
    <path d='M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z' />
  </svg>
)

export const RemoveSvg = (): JSX.Element => (
  <svg xmlns='http://www.w3.org/2000/svg' height='36px' viewBox='0 0 24 24' width='36px' fill='#000000'>
    <path d='M0 0h24v24H0V0z' fill='none' />
    <path d='M19 13H5v-2h14v2z' />
  </svg>
)
